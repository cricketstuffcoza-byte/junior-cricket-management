...
export function LiveScoreClient({initialData}:{initialData:Feed}){
 ...
 return <div className="shell"><header className="topbar"><div className="brand"><div className="brand-mark">J</div><div>Junior Cricket Management<small>Live Scoring</small></div></div><div className="pill">LIVE</div></header><main className="main">
  ...
  <section className="card section"><div className="section-title">Ball-by-ball</div>{grouped.length===0?<div className="empty">No scoring events yet.</div>:<div style={{display:'grid',gap:6}}>{grouped.map(e=><div key={e.sequence_number} className="live-event-row"><strong className="live-event-ball">{e.innings_no===2?'2nd':'1st'} · {e.over_no}.{e.ball_no}</strong><span className="live-event-players"><strong>{e.bowler_name||'—'}</strong><span className="live-event-to"> to </span><strong>{e.striker_name||'—'}</strong><small>{eventText(e)}</small></span><strong className="live-event-runs">{e.runs}</strong></div>)}</div>}</section>
 </main></div>
}
