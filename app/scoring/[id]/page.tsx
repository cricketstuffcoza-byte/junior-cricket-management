...
export default async function ScoringPage({params}:{params:Promise<{id:string}>}){
  ...
  const matchResult = await supabase.from('jcm_matches').select('id,team_a_id,team_b_id,age_group,ruleset,status,toss_winner_team_id,toss_decision,scheduled_at,result_winner_team_id,result_reason,result_margin').eq('id',id).single();
  const match = matchResult.data;
  if(!match) redirect('/operations');

  const [stateResult, inningsResult, matchPlayersResult, eventsResult, statsResult, teamsResult] = await Promise.all([
    ...
    supabase.from('jcm_teams').select('id,name,school_id').in('id',[match.team_a_id,match.team_b_id])
  ]);
  ...
  if(!state) redirect('/operations');
  const players=(matchPlayers as any[]).map((p)=>({id:p.player_id,team_id:p.team_id,batting_order:p.batting_order,name:p.jcm_players?`${p.jcm_players.first_name} ${p.jcm_players.surname}`:'Player'}));
