...
export default async function MatchPreparation({params}:{params:Promise<{id:string}>}){
 ...
 const [{data:m},{data:teams},{data:matchPlayers},{data:competitions}]=await Promise.all([
  supabase.from('jcm_matches').select('id,fixture_id,team_a_id,team_b_id,age_group,ruleset,scheduled_at,status,toss_winner_team_id,toss_decision,competition_id').eq('id',id).single(),
