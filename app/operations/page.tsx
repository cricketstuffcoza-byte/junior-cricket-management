...
export default async function OperationsPage() {
  ...
  if (!isAdmin && !(links ?? []).some(x => ['SCHOOL_ADMIN', 'COACH'].includes(x.role))) {
    redirect('/dashboard');
  }
    ...
    <div className="shell">
      <header className="topbar">
        <div className="brand">
          ...
          <div>Junior Cricket Management<small>Cricket Operations</small></div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <span className="pill">{isAdmin ? 'SYSTEM ADMIN' : 'SCHOOL OPERATIONS'}</span>
      ...
      <div className="layout">
        <aside className="sidebar">
          ...
          <a className="nav-item active" href="/operations">Competitions & Fixtures</a>
          <a className="nav-item" href="/dashboard">Dashboard</a>
          <div className="nav-label" style={{ marginTop: 14 }}>Match flow</div>
          <a className="nav-item" href="#matches">Matches</a>
          <a className="nav-item" href="#matches">Live Scoring</a>
        </aside>

        <main className="main">
          <div className="banner">
            <h2>Cricket Operations</h2>
            <p>Build the fixture pipeline from competition and schedule through availability and squad selection.</p>
          </div>
          <div className="eyebrow">Competition & fixture management</div>
          <h1>Schedule the cricket</h1>
          <p className="subtitle">Competitions, fixtures and the availability workflow now share one JCM operational layer.</p>
          ...
          <section id="matches" className="card section">
            <div className="section-title">Matches <span className="count">{matchRows.length}</span></div>
            {matchRows.length === 0 ? (
              <div className="empty">No matches prepared yet.</div>
            ) : (
              <div className="table-wrap">
                <table>
                  <thead>
                    <tr><th>Match</th><th>Age</th><th>Ruleset</th><th>Status</th><th>Scheduled</th><th /></tr>
                  </thead>
                  <tbody>
                    {matchRows.map(m => {
                      ...
                      return (
                        <tr key={m.id}>
                          ...
                          <td style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                            <a className="secondary-button" href={`/operations/matches/${m.id}`}>
                              {ready ? 'Open preparation' : 'Prepare'}
                            </a>
                            {ready && <a className="button" href={`/scoring/${m.id}`}>Live scoring</a>}
                          </td>
                        </tr>
                      );
                    })}
