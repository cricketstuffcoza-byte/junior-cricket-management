        ...
        <section id="matches" className="card section"><div className="section-title">Matches <span className="count">{matchRows.length}</span></div>
          {matchRows.length===0 ? <div className="empty">No matches prepared yet.</div> : <div className="table-wrap"><table><thead><tr><th>Match</th><th>Age</th><th>Ruleset</th><th>Status</th><th>Score</th><th>Scheduled</th><th/></tr></thead><tbody>
            {matchRows.map(m=>{
              ...
              return <tr key={m.id}><td><strong>{teamName(m.team_a_id)} vs {teamName(m.team_b_id)}</strong><small>{league}</small><small>📍 {venue}</small><small>{matchLabel}</small></td><td>{m.age_group}</td><td>{m.ruleset}</td><td><span className="status">{m.status}</span></td><td>{score ? <strong>{score}</strong> : <span style={{color:'var(--muted)'}}>—</span>}</td><td>{m.scheduled_at?new Date(m.scheduled_at).toLocaleString('en-ZA'):'—'}</td><td style={{display:'flex',gap:6,flexWrap:'wrap'}}><a className="secondary-button" href={'/operations/matches/'+m.id}>{ready?'Open preparation':'Prepare'}</a>{ready&&<a className="button" href={'/scoring/'+m.id}>Live scoring</a>}</td></tr>;
            })}
          </tbody></table></div>}
        </section>
      </main>
